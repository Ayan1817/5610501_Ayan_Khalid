...
int cookies(int k, int A_count, int* A) {
    MinHeap heap;
    heap.data = malloc(A_count * sizeof(int));
    heap.size = 0;
    
    // Build heap
    for(int i=0;i<A_count;i++) insertHeap(&heap, A[i]);
    
    int operations = 0;
    while(heap.size > 1 && heap.data[0] < k) {
    ...
    if(heap.size > 0 && heap.data[0] >= k) return operations;
    return -1;
}
